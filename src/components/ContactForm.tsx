...
export default function ContactForm() {
  ...
  const validate = (): FormErrors => {
    ...
    if (!form.phone.trim()) {
      ...
      errs.phone = "Enter a valid Egyptian phone number";
    }
    if (!form.email.trim()) {
  ...
  return (
    ...
    >
      ...
      <div className="absolute top-0 right-0 w-96 h-96 bg-gold/5 rounded-full -translate-y-1/2 translate-x-1/2 float-animation" />
      <div className="absolute bottom-0 left-0 w-64 h-64 bg-gold/5 rounded-full translate-y-1/2 -translate-x-1/2 float-animation-reverse" />
                ...
                <>
                  ...
                  <form onSubmit={handleSubmit} className="space-y-5" noValidate>
                    ...
                    <button
                      type="submit"
                      disabled={loading}
                      className="w-full btn-gold py-4 rounded-xl text-sm font-semibold tracking-wider uppercase mt-2 disabled:opacity-70 flex items-center justify-center gap-2"
                    >
                      {loading ? (
                        <>
                          <svg
